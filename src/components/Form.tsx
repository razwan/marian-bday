...
const ShortAnswerInput = ({ id }: IFormInputs) => {
  ...
  return (
    <div className="space-y-1">
      ...
      <input
        className="w-full border border-neutral-300 py-2 px-4 rounded-md"
        type={"text"}
        {...register()}
      />
    </div>
...
const LongAnswerInput = ({ id }: IFormInputs) => {
  ...
  return (
    <div className="space-y-1">
      <div className="font-bold">{label}</div>
      <textarea
        className="w-full border border-neutral-300 p-4 rounded-md"
        {...register()}
      />
    </div>
  );
      ...
      <GoogleFormProvider {...methods}>
        <form id="ContactForm" onSubmit={methods.handleSubmit(onSubmit)}>
          <div className="space-y-8">
            ...
            <ShortAnswerInput id="389438953" />
            <LongAnswerInput id="1850514433" />

            <div>
              <button
                className="rounded-md bg-teal-800 text-teal-50 py-2 px-4"
                type="submit">
                Trimite
              </button>
            </div>
